import { parseJwtPayload, throwIfNot2xx, bufferToBase64 } from "./utils.js";
import { configure, type MinimalResponse } from "./config.js";
            ...
            : cognitoIdpEndpoint,
        {
            ...
            body: JSON.stringify({
                ...
                AuthParameters: {
                    ...authParameters,
                    ...(clientSecret && {
                        SECRET_HASH: await calculateSecretHash(
...
export async function revokeToken({
    ...
    const { fetch, cognitoIdpEndpoint, proxyApiHeaders, clientId } =
        configure();
    return fetch(
        cognitoIdpEndpoint.match(AWS_REGION_REGEXP)
            ? `https://cognito-idp.${cognitoIdpEndpoint}.amazonaws.com/`
    ...
    for (;;) {
        if (isAuthenticatedResponse(authResponse)) {
            return {
                ...
                expireAt: new Date(
                    Date.now() +
                        authResponse.AuthenticationResult.ExpiresIn * 1000,
                ),
        ...
        if (authResponse.ChallengeName === "SMS_MFA") {
            ...
            throw new Error(
                `Unsupported challenge: ${authResponse.ChallengeName}`,
            );
        }
...
async function calculateSecretHash(username?: string) {
    ...
    if (!username) {
        throw new Error(
            "Failed to determine username for calculating secret hash",
        );
    }
