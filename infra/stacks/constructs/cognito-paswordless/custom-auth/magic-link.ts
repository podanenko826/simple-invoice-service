...
async function createEmailContent({
    ...
    return {
        html: `<html><body><p>Your secret sign-in link: <a href="${secretLoginLink}">sign in</a></p>This link is valid for ${Math.floor(
            config.secondsUntilExpiry / 60
        )} minutes<p></p></body></html>`,
        text: `Your secret sign-in link: ${secretLoginLink}`,
        subject: "Magic Link",
    };
}
    ...
    await ddbDocClient
        .send(
            new PutCommand({
                ...
                ConditionExpression:
                    "attribute_not_exists(#iat) or #iat < :iat",
...
async function verifyMagicLink(
    ...
    let dbItem: Record<string, unknown> | undefined;
    try {
        const salt = requireConfig("salt");
